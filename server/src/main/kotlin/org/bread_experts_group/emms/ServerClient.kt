...
class ServerClient(
	...
	private val lastPing = AtomicInteger()

	private var keepAliveThread: Thread? = null
	private val keepAliveLambda = {
		while (!exit) {
			try {
				Thread.sleep(5_000)
				when (state) {
					...
					else -> throw IllegalStateException("Keep-Alive thread present in illegal state")
				}
	...
	private var mailboxThread: Thread? = null
	private val mailboxLambda = {
		try {
			while (!exit) {
				Thread.sleep(9999_9999_9999)
			}
		} catch (_: InterruptedException) {
		}
	}
					...
					State.LOGIN -> when (packetID) {
						...
						0x03 -> {
							...
							keepAliveThread = Thread.ofVirtual()
								.name(Thread.currentThread().name + " keep-alive thread")
								.start(keepAliveLambda)
						}
					...
					State.CONFIGURATION -> when (packetID) {
						...
						0x05 -> {
							...
							if (expected != actual) disconnect(
								Component.Literal("Ping returned incorrect result. Expected $expected, got $actual.")
							)
						}
